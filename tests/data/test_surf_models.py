...
def test_surf_keys_is_exactly_the_prd_contract() -> None:
    ...
    **192 = 83 + 71 + 5 + 33**: the 83 that shipped through v0.8.3, the ``p``
    ...
    bodies' thirty-three in ``SWARM_KEYS`` -- eighteen added 2026-09-16, plus
    ...
    followed by seven BOARD keys (191) and the served health status word (192). The count is asserted
    ...
    assert len(SURF_KEYS) == len(set(SURF_KEYS)) == 192
