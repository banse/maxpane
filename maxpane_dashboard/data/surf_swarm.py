...
__all__ = [
    ...
    "unfinished_ids",
    # swarm v2 (WP3); the names above stay until WP7 retires their widgets
...
# ---------------------------------------------------------------------------
# swarm v2 (plan WP3, 2026-09-21).  Everything above stays byte-identical
# until WP7 retires the old widgets; everything below is what the rebuilt
# ``s`` body and the new ``a`` AGENT body read.
#
...
# ``[]`` (looked, nothing there) from whether the read happened -- the rule
# ``throughput`` above documents, kept here so no fold can be talked into
# faking a read by being handed ``None``.
# ---------------------------------------------------------------------------
