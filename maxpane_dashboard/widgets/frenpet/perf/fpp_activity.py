"""Battle activity feed for the FrenPet Performance view.
...
class FPPerfActivity(Vertical):
    """Scrolling activity feed showing battles involving wallet pets."""

    DEFAULT_CSS = """
    FPPerfActivity > .fpp-feed-title {
        width: 100%;
        padding: 0 1;
        text-style: bold;
        color: $text-muted;
    }
    FPPerfActivity > RichLog {
        height: 1fr;
        padding: 0 1;
        scrollbar-size: 1 1;
    }
    ...
    def compose(self) -> ComposeResult:
        yield Static("ACTIVITY", classes="fpp-feed-title")
        yield RichLog(id="fpp-activity-log", wrap=True, highlight=True, markup=True)
    ...
    def update_data(
        ...
        log = self.query_one("#fpp-activity-log", RichLog)
