...
| A8 | §2/§4: THE LIST gains a **second view** behind `y` — the reader's own standing | added 2026-08-17 on request. §2 scoped the wallet to a single YOU row and that shipped; this is the follow-on, and it is presentation over data already in hand — the six wallet views the fast tier reads and the `Deposited` history the log sweep holds, so the view adds **no request**. Layout: its **own hero** (YOUR RANK · YOUR SCORE · YOUR NEXT SEND — the game's CLOCK/LIST/CURVE is about the list, and in this view the reader is asking about themselves; the phase and hour stay legible in the title bar, and the EOA subtitle stays under both heroes), then the ladder on the left and a rail of three stacked panels on its right (revised 2026-08-17 on the reader's own screenshot — the three were briefly two rows of two, and reading them top to bottom as one thought works better against a tall ladder). YOUR LADDER (every send, the multiplier it got, and whether it credited or was above the cap), YOUR STANDING (rank, score, banked weight, share of all weight, when it joined), YOUR NEXT MOVE (the requirement alone: `requiredNext` and the high-water mark it must beat) and WHERE IT GETS YOU (the consequence: the points that send buys, whether it is **enough to move up**, and what the rank above would cost). The requirement and its consequence are two different thoughts, and a reader deciding what to type wants the first one alone — which is also what made room for the honest third state: `you_next_send_passes` is `None` when the comparison could not be made, and rendering that as "not enough" would be a claim about a number nobody read. `y` swaps the **body** and leaves the hero in place, so the doomsday clock never leaves the screen — the view exists to decide what to send, and that decision is worthless without the clock it is racing. One label column across the whole rail, with a comparison glyph spent out of the two-column gutter so `≥ 491.00`, `490.90` and `4 of 10,911` all start their value in the same column; each labelled line's **headline value is bold and coloured** while its tail is not, so the eye lands on the number that matters. The ladder's last column is the running **score after each rung**, which is the concave curve made visible (450 ETH buys 29,540 points; the next 0.1 ETH escalations buy three each) and the first column the panel sheds, being the only derived one. The status bar names this screen's own `c` and `y` — bought from `tab switch` and `updated Ns ago`, never from the error count, and the freshness that matters is the title bar's `as of HH:MM`, which freezes under an outage where the cycle age keeps counting. Nine new keys and one new row shape (`you_ladder_rows`, source-backed: `None` when the logs pool did not read, because an empty ladder rendered as fact tells a depositor they never deposited). Measured: the view clears at the same **138** columns, so nothing moved |
| A9 | §2's **"No ENS resolution"** is reversed: wallets render their verified reverse-ENS name wherever one exists | added 2026-08-17 on request. The scope line was written when this would have been a new subsystem; `data/ens.py` already had the whole thing — keyless reverse resolution in four batched `aggregate3` rounds with the **forward check** that stops any address claiming `vitalik.eth`, since setting a reverse record needs nobody's permission. So this is wiring: a Multicall3 batch on the client's own state pool, a TTL store (names 6 h, misses 1 h — most wallets have no record and without the miss half every one of them is re-queried forever), and a name column on the three tables plus `you_ens` / `whale_ens` / `last_saved_ens`. Two constraints shape the render: a name is the most attacker-controlled string on the screen, so it is escaped like any other; and it is capped at **`NAME_COLS = 12`** in tables — measured, because at 15 the full layout went 138 → **144**, past the 143 FWA sets. 12 is `surfsurf.eth` exactly and leaves the measured 138 untouched. The full name is shown whole in the new **YOUR WALLET** panel, which also answers the question every other panel begged: *which* wallet is this page about |
| A10 | `y` with **no wallet configured** asks for one, then opens the view that was asked for | added 2026-08-17 on request. Every panel in the `y` view is about a wallet, so with none set it opened as six panels of `--` and left the reader to guess that a *different* key (`w`) was the one they wanted. `y` now pushes the same `WalletInputScreen`; entering an address opens the wallet view, escaping leaves the reader on the dashboard. `w` alone still only sets the wallet — it answers *which wallet*, where `y` asks *show me mine*. Two honesty fixes came out of driving it live: the address is repainted **immediately** rather than at the end of the next cycle (a panel still reading `press w to set one` right after you set one reads as a keypress that failed), and in that window the ENS line says `resolving…` rather than `no ENS record`, which would be a confident negative about a lookup that has not run — and wrong for exactly the reader who just typed a wallet that does have a name. The YOU row's copy now names what the key *does* (`no wallet set · press w to set one · or MAXPANE_WALLET`), in three parts so a narrow rail sheds the env var — the fix you cannot do from here — first |
