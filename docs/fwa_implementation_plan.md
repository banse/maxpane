...
| Multicall3 ≤ 500 calls per `eth_call` | Measured: 1 aggregate + 9 slot batches + 8 listing batches = **18 `eth_call`s** at 3,867 positions, **26** at today's 5,942. Scaling law `1 + ceil(1.073N/500) + ceil(N/500)` ≈ **18 at 3,867 / 26 at 5,942 / 43 at 10,000**. ≈3.3 s per sweep — too heavy for a 15 s tick. |
...
|---|---|---|---|
| Fast | 15 s | `quoteAcquisitionPrice`, `acquisitionFee`, pool temperature (`lastAcquisitionTs`), VRF queue depth, crown pot, gate flags, `eth_getBalance(core)` | **3** `eth_call`s / 5 round trips, independent of N: the aggregate3 batch (~30 views across 8 contracts) + `quoteAcquisitionPrice` (kept out of the batch — bounding gas for one view is not bounding it for 48) + `tokenShareBps(gap)` (needs an argument the batch itself reads), plus `eth_getBalance` and a cached `eth_gasPrice`. Drops to 1 only if both follow-ups are disabled. |
| Medium | 60 s | full position enumeration → odds board, chase board, Pull EV | **18** `eth_call`s at 3,867 / **26** at 5,942 (`1 + ceil(1.073N/500) + ceil(N/500)` ≈ **18 at 3,867 / 26 at 5,942 / 43 at 10,000**), ~3.3 s, single-flight |
...
| After WP-14 | **The budget is a function of the live position count `N`, not a constant** — the pool grew +53 % in two days (3,867 → 5,942), so a fixed gate expires within the week. **Fast tick = 3 `eth_call`s** (1 bare; `quoteAcquisitionPrice()` needs its own bounded `gas`/`gasPrice` and `tokenShareBps(gap)` needs an argument the batch itself reads, so neither can ride the aggregate3) + `eth_getBalance` + TTL-cached `eth_gasPrice` = **5 round trips, independent of `N`**. **Sweep ≤ `1 + ceil((1.2·N + 64) / 500) + ceil(N / 500)` `eth_call`s** plus 1 `eth_blockNumber`; measured **18 at N=3,867 · 26 at N=5,942 · 43 at N=10,000** (≈ `1 + N/241`). §8.4's "~12" and findings §6.1's "17" both undercount — the sweep at 3,867 is 1 aggregate read + 9 slot batches + 8 listing batches = 18 — and the old fixed "≤ 20" gate is already crossed at N≈4,500, i.e. ~0.7 days after the capture it was written from. No Multicall3 chunk exceeds 500 calls (boundary-tested at 499/500/501/1000/1001); no raw JSON-RPC array batching at all (the ≤ 60-element ceiling holds by construction); 0.12 s inter-call pacing observed on a fake clock. Overlapping sweeps are **skipped, not queued**, verified under real concurrency, and the published snapshot is atomic — a consumer polling every event-loop turn *and* every wire request only ever sees a whole block-consistent sweep. Decode/aggregate CPU ≈ 10 µs/position (~37 ms at 3,867, ~1 % of a live sweep): the tier is latency-bound, not CPU-bound. **Growth headroom — the number that matters: the 60 s medium tier stops fitting at N≈12,900 at 1 s/round-trip (≈3.6 days at the observed +24 %/day), N≈26,500 at 0.5 s, N≈77,000 at the calibrated 0.17 s.** All of the above asserted by `tests/data/test_fwa_refresh_budget.py` (30 tests). |
